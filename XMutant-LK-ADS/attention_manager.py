...
class AttentionManager:
    def __init__(
        ...
        logger.debug(
            f"get every {index_step}th element starting at index 0; new size is {len(self.df_heat_map)}"
        )
    ...
    def control_point_selection(self, factor=40):
        ...
        mean_avg_heatmaps = (
            self.df_heat_map.groupby("closest_cp")["avg_heatmaps"].mean().reset_index()
        )
        ...
        self.df_heat_map = pd.merge(
            self.df_heat_map, mean_avg_heatmaps, on="closest_cp", how="left"
        )

        mean_average_gradient = (
            self.df_heat_map.groupby("closest_cp")["avg_gradient_heatmaps"].mean().reset_index()
        )
        avg_gradient_heatmaps = mean_average_gradient["avg_gradient_heatmaps"].to_list()

        mean_average_gradient["weights_by_gradient"] = np.exp(
            avg_gradient_heatmaps / np.sum(avg_gradient_heatmaps) * factor
        )
        mean_average_gradient.rename(
            columns={"avg_gradient_heatmaps": "avg_gradient_heatmaps_by_cp"}
        )
        self.df_heat_map = pd.merge(
            self.df_heat_map, mean_average_gradient, on="closest_cp", how="left"
        )

        cp_id = random.choices(
            population=mean_average_gradient["closest_cp"],
            weights=mean_average_gradient["weights_by_gradient"],
            k=1,
        )[0]
...
if __name__ == "__main__":
    ...
    attention_manager = AttentionManager(
        simulation_name=episode_sub_path6, index_step=10, attention_type="Faster-ScoreCAM"
    )
