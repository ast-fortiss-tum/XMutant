...
class AttentionManager:
    def __init__(self,
        ...
        logger.debug(f"get every {index_step}th element starting at index 0; new size is {len(self.img_paths) }")
    ...
    def control_point_selection(self, factor=40):
        ...
        avg_gradient_heatmaps_by_cp = np.array([np.mean(self.avg_gradient_heatmaps[self.closest_cps == cp])
                                                for cp in unique_cp])
...
if __name__ == "__main__":
    ...
    attention_manager = AttentionManager(simulation_name=episode_sub_path7,
                                        index_step=10,
