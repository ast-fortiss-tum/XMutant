...
def average_intensity(
    heat_map, green_mask, yellow_mask, white_mask, road_mask, right_lane_mask, detail=DETAIL
):

    average_intensity_right_lane = (
        np.mean(heat_map[right_lane_mask > 0]) if np.sum(right_lane_mask) > 0 else np.nan
    )
    average_intensity_left_lane = (
        np.mean(heat_map[road_mask - right_lane_mask > 0])
        if np.sum(road_mask - right_lane_mask) > 0
        else np.nan
    )
    average_intensity_yellow = (
        np.mean(heat_map[yellow_mask > 0]) if np.sum(yellow_mask) > 0 else np.nan
    )
    average_intensity_white = (
        np.mean(heat_map[white_mask > 0]) if np.sum(white_mask) > 0 else np.nan
    )
    average_intensity_green = (
        np.mean(heat_map[green_mask > 0]) if np.sum(green_mask) > 0 else np.nan
    )
...
def check_one_episode(simulation_name, xai_method, index_step=INDEX_STEP):
    ...
    heat_map = attention_manager.single_attention_map(
        preprocessed_image=processed_images_yuv, attention_type=xai_method
    )
    ...
    for idx in range(processed_images.shape[0]):
        ...
        av_rl, av_ll, av_y, av_w, av_g = average_intensity(
            heat_map[0],
            green_mask,
            yellow_mask,
            white_mask,
    ...
    print(
        f"mean of re_ll_list {np.nanmean(re_ll_list)} variance of re_ll_list {np.nanvar(re_ll_list)}"
    )
...
def enumerate_sim_folder(timestamp=None, detail: bool = DETAIL):
    ...
    if timestamp is None:
        ...
        collection_df.to_csv(
            "./segmentation/heat_map_ranking_" + timestamp[0] + "-" + timestamp[1] + ".csv",
            index=False,
        )
...
def check_one_sim(folder_path: str, detail: bool = DETAIL):
    ...
    for episode_folder in episode_folders:
        ...
        for xai_method in XAI_methods:
            ...
            mean_list, var_list, mean_re_list, var_re_list = check_one_episode(
                episode_folder, xai_method, index_step=INDEX_STEP
            )
...
def bar_plot(df_name: str):
    ...
    xai_ranking_result_selected = xai_ranking_result[
        [
            "mean_rl",
            "mean_ll",
            "mean_y",
    ...
    plt.legend(
        [
            "mean of intensity on right lane",
    ...
    xai_ranking_result_re = xai_ranking_result[
        [
            "mean_re_ll",
            "mean_re_y",
            "mean_re_w",
