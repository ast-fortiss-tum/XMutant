...
from utils.road_utils import (  # get_closest_control_point,
    get_closest_previous_control_point,
)
...
parser.add_argument(
    "--mutation-type",
    help="XAI to 'high' or 'low' attention direction, or 'random'",
    type=str,
    default="RANDOM",
)
parser.add_argument(
    "--mutation-method",
    help="mutation to 'high' or 'low' attention direction, or just 'random'",
    type=str,
    choices=["random", "attention_same", "attention_opposite"],
    default="random",
)
...
def perform_one_test(
    seed: int = 0,
    mutation_type: str = args.mutation_type,
    mutation_method: str = args.mutation_method,
) -> bool:
    """
    ...
    while episode_count < args.num_episodes:
        ...
        if mutation_point is not None:
            logger.debug(
                "episode_count %d, mutation_point: %d, direction: %s"
                % (episode_count, mutation_point, mutation_direction)
            )

        time.sleep(1)

        obs = env.reset(mutation_info=[mutation_point, mutation_direction], skip_generation=False)
        time.sleep(1)
        ...
        while not done:
            ...
            if done:
                ...
                csv_logger.episode_logger(
                    os.path.join("simulations", "global_log.csv"), global_log_info
                )
                ...
                if info["is_success"] == 1:
                    ...
                    if mutation_type.upper() == "XAI":
                        ...
                        attention_manager = AttentionManager(
                            simulation_name=episode_sub_path,
                            index_step=10,
                        ...
                        logger.debug(
                            "closest control point to the hottest point is %d\n" % mutation_point
                        )
...
def main(number_of_test=1):
    ...
    for i in range(number_of_test):
        ...
        result = perform_one_test(
            seed=seed_, mutation_type=args.mutation_type, mutation_method=args.mutation_method
        )
        """if not result:
