...
import numpy as np
from config import (  # MAX_ANGLE,; NUM_CONTROL_NODES,; SEG_LENGTH,
    DISPLACEMENT,
    NUM_SAMPLED_POINTS,
    ROAD_WIDTH,
)
...
def get_closest_previous_control_point(
    point: Tuple[float, float], cp: List[Tuple[float, float]]
) -> int:
    nodes = np.asarray(cp)
    dist_2 = np.sum((nodes - point) ** 2, axis=1)
...
def mutate_road(
    ...
    if method is None:
        ...
        if index == 0:
            ...
            point2 = np.array(cp[index + 1])
        orthogonal_left, orthogonal_right, orthogonal_increase = orthogonal_direction(
            temp, point1, point2
        )
...
def is_valid(control_nodes, sample_nodes):
    return RoadPolygon.from_nodes(sample_nodes).is_valid() and RoadBoundingBox(
        bbox_size=(0, 0, 250, 250)
    ).contains(RoadPolygon.from_nodes(control_nodes[1:-1]))
...
if __name__ == "__main__":
    ...
    closest = get_closest_control_point(
        tuple([point_start[0], point_start[1]]),
