...
def load_mnist_test(popsize, number):
    ...
    if popsize < filtered_test_y.shape[0]:
        select_index = np.random.choice(
            range(filtered_test_x.shape[0]), size=popsize, replace=False
        )
...
class Population:
    def __init__(self, number, pop_size=POPSIZE, xai_method=ATTENTION, enable_timer: bool = False):
        # x_test, y_test = load_dataset(pop_size)
        self.digit = number

        x_test, y_test = load_mnist_test(pop_size, self.digit)
        self.population_to_mutate = [
            generate_digit(i, image, label) for i, (image, label) in enumerate(zip(x_test, y_test))
        ]
    ...
    def evaluate_population(self, gen_number, folder):
        ...
        predictions, confidences = Predictor.predict(img=batch_individual, label=batch_label)
        # label result and detect misclass
        for ind, prediction, confidence, attmap in zip(
            self.population_to_mutate, predictions, confidences, attmaps
        ):
    ...
    def create_report(self, path, gen_number):
        ...
        with open(dst, mode="w") as report_file:
            ...
            report_writer.writerow(
                [self.size, gen_number, self.misclass_number, CONTROL_POINT, MUTEXTENT]
            )
