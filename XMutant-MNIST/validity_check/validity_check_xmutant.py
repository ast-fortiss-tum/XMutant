...
def reconstruction_probability(dec, z_mean, z_log_var, X):
    ...
    reconstruction_loss = tf.reduce_mean(
        tf.reduce_sum(tf.keras.losses.binary_crossentropy(X, mu_hat), axis=(-1))
    )
...
def check_validity_rate_over_iteration(record_name, digit=None):
    ...
    df_cumulative_validity["idx"] = (
        df_record.groupby("mutation_number")["ID_0.9"].sum().to_frame().index
    )

    for confidence, threshold in VAE_THRESHOLDS.items():
        df_cumulative_validity["pop_num"] = (
            df_record.groupby("mutation_number")["one"].sum().to_list()
        )
        df_cumulative_validity["pop_cum_num"] = (
            df_record.groupby("mutation_number")["one"].sum().cumsum().to_list()
        )
...
if __name__ == "__main__":
    ...
    for csv_file in folders:
        ...
        check_validity_rate_over_iteration(
            record_name=csv_file, digit=None
        )  # get cumulative validity rate
