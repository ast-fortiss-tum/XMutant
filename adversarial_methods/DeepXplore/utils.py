...
def constraint_occl(gradients, start_point, rect_shape):
    ...
    new_grads[
        :,
        start_point[0] : start_point[0] + rect_shape[0],
...
def neuron_to_cover(model_layer_dict):
    not_covered = [
        (layer_name, index) for (layer_name, index), v in model_layer_dict.items() if not v
    ]
    if not_covered:
...
def calculate_density(x_target_orig, vae):
    ...
    reconstructed_prob_x_target = reconstruction_probability(
        vae.get_layer("decoder"), z_mean, z_log_var, x_target_orig
    )
