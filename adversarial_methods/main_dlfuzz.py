...
if __name__ == "__main__":
    ...
    for digit in range(10): # range(10):
        ...
        for i, (tmp_img,image_label) in enumerate(zip(x_test, y_test)):
            ...
            prediction, confidence = Predictor.predict_single(np.expand_dims(tmp_img, 0), image_label)
            if prediction!= image_label:
                ...
                prediction, confidence = Predictor.predict_single(np.expand_dims(gen_img, 0), image_label)
                if prediction!= image_label:
                    print(f"succeed misclass id {i}, label {image_label}, predict {prediction, confidence}")
